...
const parseOperation = (operation: string[], worryLevel: number): number => {
...
const product = monkeys.map(({ divisor }) => divisor).reduce((acc, n) => acc * n)
...
const playRound = () => {
  for (const monkey of monkeys) {
    ...
    for (const item of items) {
      ...
      const worryLevel = parseOperation(operation, item) % product
